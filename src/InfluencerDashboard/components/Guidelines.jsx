        ...
        <div className="relative bg-white shadow-2xl rounded-3xl p-6 sm:p-12 z-10">
          ...
          {campaignData.images?.length > 0 && (
            <form
              onSubmit={(e) => {
                e.preventDefault();
                const storyLinks = Array.from(e.target.elements)
                  .filter((elem) => elem.name === 'storyLink' && elem.value)
                  .map((elem) => elem.value);
                console.log('Submitted Story Links:', storyLinks);
              }}
              className="mb-6"
            >
              <h3 className="text-xl font-semibold text-gray-800 mb-4">Submit Story Links</h3>
              {[...Array(Math.min(campaignData.images.length, 5))].map((_, index) => (
                <div key={index} className="mb-4">
                  <label
                    htmlFor={`storyLink${index + 1}`}
